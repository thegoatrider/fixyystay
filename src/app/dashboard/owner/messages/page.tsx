...
import { Send, ShieldAlert, AlertCircle, Paperclip, Loader2, X, FileText } from 'lucide-react'
...
export default function OwnerMessagesPage() {
  ...
  async function handleSendMessage(e: React.FormEvent) {
    ...
    const res = await sendMessage(ownerId, 'owner', content, attachmentUrl)
    if (!res.success) {
      setMessages(prev => prev.filter(m => m.id !== tempMsg.id))
    ...
    <div className="max-w-4xl mx-auto h-[calc(100vh-8rem)] mt-6 flex flex-col bg-white rounded-2xl shadow-sm border overflow-hidden">
      ...
      <div className="p-4 border-t bg-white">
        <div className="max-w-4xl mx-auto">
          {selectedFile && (
            <div className="flex items-center gap-2 mb-3 p-2 bg-blue-50 rounded-lg text-sm text-blue-700 w-fit">
          ...
          <div className="flex gap-2 items-center">
            ...
            <form onSubmit={handleSendMessage} className="flex-1 flex gap-2">
              <Input 
                ...
                disabled={isUploading}
              />
              <Button type="submit" size="icon" className="rounded-full h-12 w-12 shrink-0 bg-blue-600 hover:bg-blue-700 shadow-md" disabled={(!newMessage.trim() && !selectedFile) || isUploading}>
